...
var PopUpPool = {};
var POPUP = function (path, onshow) {
    let box = this;
    XHR.push({
        method: "GET",
        addressee: path,
        onsuccess: function (response) {
            ...
            box.body = box.window.querySelector(".pop-up-body") || null;
            ...
            for (var handle in PopUpPool) {
                if (PopUpPool.hasOwnProperty(handle)) {
...
function swapRow(row) {
    row.swap(row.previousElementSibling);
}
...
function showPrompt(message, callback, placeholder, defValue) {
    var template = document.querySelector("#prompt").cloneNode(true);
    var form = template.content.firstElementChild;
    form.message.value = message;
    form.field.value = defValue || "";
    document.body.appendChild(form);
