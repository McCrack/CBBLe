<form class="p-10px w-50 border-box @if(!$privileges) disabled @endif" autocomplete="off">
    
    <p class="font-14 primary-txt text-bold">Global:</p>
    
    <div class="m-5">
        <label class="block">{{ $translate->main('default theme') }}</label>
        <div class="select w-100">
            <input name="theme" value="{{ ($global['theme'] ?? null) }}" class="field w-100 cursor-pointer" readonly>
            <fieldset
    ...
    <div class="m-5">
        <label class="block">{{ $translate->main('default language') }}</label>
        <div class="select w-100">
            <input name="language" value="{{ ($global['language'] ?? null) }}" class="field w-100 cursor-pointer" readonly>
            <fieldset
